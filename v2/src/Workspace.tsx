...
function AdvancePeoplePage({ store }: { store: Store }) {
  ...
  return <main className="workspace-page">
    ...
    <form className="inline-add advance-person-add" onSubmit={(event) => { event.preventDefault(); void addPerson() }}><input aria-label="新增常用姓名" placeholder="輸入姓名" value={name} onChange={(event) => setName(event.target.value)} /><button type="submit"><Plus />新增</button></form>
    {error ? <p className="form-error" role="alert">{error}</p> : null}
